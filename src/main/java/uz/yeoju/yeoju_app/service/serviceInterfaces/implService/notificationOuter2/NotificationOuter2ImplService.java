...
public class NotificationOuter2ImplService implements NotificationOuter2Service {
    private final NotificationOuter2Repository notificationRepository;
    private final NotificationOuter2CounterRepository notificationOuterCounterRepository;
    ...
    public ApiResponse getStudentNotifications(String studentId) {
        ...
        for (GetStudentNotificationOuters outer : notificationRepository.getStudentNotificationOuters(studentId)) {
            Boolean exists = notificationOuterCounterRepository.existsByUserIdAndNotificationOuter2Id(studentId, outer.getId());
            if (!exists) {
                Optional<User> userOptional = userRepository.findById(studentId);
                if (userOptional.isPresent()){
                    Optional<NotificationOuter2> outerOptional = notificationRepository.findById(outer.getId());
                    if (outerOptional.isPresent()){
                        User user = userOptional.get();
                        NotificationOuter2 notificationOuter = outerOptional.get();
                        Long aLong = notificationOuterCounterRepository.maxQueue();
                        if (aLong==null || aLong<1) {
                            notificationOuterCounterRepository.save(new NotificationOuter2Counter(
                                    1L,
                                    user,
                                    notificationOuter
                            ));
    ...
    public ApiResponse delete(User user, String id) {
        Optional<NotificationOuter2> outerOptional = notificationRepository.findById(id);
        if (outerOptional.isPresent()){
            NotificationOuter2 notificationOuter = outerOptional.get();
            if (Objects.equals(notificationOuter.getCreatedBy(), user.getId())){
                Boolean b = notificationOuterCounterRepository.existsByUserIdAndNotificationOuter2Id(user.getId(), notificationOuter.getId());
                if (!b) {
    ...
    public ApiResponse update(NotificationOuterCreateDto dto){
        Optional<NotificationOuter2> outerOptional = notificationRepository.findById(dto.getId());
        if (outerOptional.isPresent()){
    ...
    public ApiResponse save(NotificationOuterCreateDto dto){
        ...
        if (optionalDekanat.isPresent()){
            ...
            if (optionalEducationYear.isPresent()){
                ...
                NotificationOuter2 notificationOuter = new NotificationOuter2();
    ...
    public GetNotificationOuterDto generateDto(NotificationOuter2 notification){
